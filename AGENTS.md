...
- 现场降级: 平台不可达时，相机本地白名单（适配器定期同步月卡/固定车位/白名单车牌）可自动放行，临停车辆由现场人工处理
...
### 通信
- 平台→适配器走 HTTP REST（命令下发），适配器→设备走 MQTT
- 适配器→平台走 HTTP Webhook（事件上报，需签名校验）
