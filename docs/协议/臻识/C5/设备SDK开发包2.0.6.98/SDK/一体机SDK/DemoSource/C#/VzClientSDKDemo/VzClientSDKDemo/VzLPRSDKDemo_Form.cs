...
namespace VzClientSDKDemo
{
    public partial class VzLPRSDKDemo_Form : Form
    {
        ...
        private VzClientSDK.VZ_COMMON_RESULT_CALLBACK m_common_recv = null;
        private int record_handle_ = 0;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            m_common_recv = new VzClientSDK.VZ_COMMON_RESULT_CALLBACK(OnCommonRecv);
        ...
        public void RecordReset(object source, System.Timers.ElapsedEventArgs e)
        {
          if (record_handle_ != 0)
          {
            VzClientSDK.VzLPRClient_StopSaveRealData(record_handle_);
            ...
            VzClientSDK.VzLPRClient_SaveRealData(record_handle_, szPath);
          }
        }
                                                  ...
                                                  IntPtr pImgPlateClip)
        {
            if (eResultType != VzClientSDK.VZ_LPRC_RESULT_TYPE.VZ_LPRC_RESULT_REALTIME )
            {
                ...
                {
                  if (lblPlate.InvokeRequired)
                  {
                    lblPlate.Invoke(new Action<Control, string>((ctrl, data) =>
                    {
                      ctrl.Text = data;
                    }), new object[] { lblPlate, strLicense });
                  }

                  if (pictureBox2.InvokeRequired)
                  {
                    pictureBox2.Invoke(new Action<PictureBox, string>((ctrl, data) =>
                    {
                      Image img = Image.FromFile(data);
                      if (img != null)
                      {
                        ctrl.Image = img;
                        ctrl.Refresh();
                    ...
                    }), new object[] { pictureBox2, path });
                  }
                }
                else
                {
                  if (lblPlate2.InvokeRequired)
                  {
                    lblPlate2.Invoke(new Action<Control, string>((ctrl, data) =>
                    {
                      ctrl.Text = data;
                    }), new object[] { lblPlate2, strLicense });
                  }

                  if (pictureBox4.InvokeRequired)
                  {
                    pictureBox4.Invoke(new Action<PictureBox, string>((ctrl, data) =>
                    {
                      Image img = Image.FromFile(data);
                      if (img != null)
                      {
        ...
        private void OnCommonRecv(int handle, int type, IntPtr pResultInfo, int len, IntPtr imgs, int count, IntPtr pUserData)
        {
          byte[] result_bytes = new byte[len];
          Marshal.Copy(pResultInfo, result_bytes, 0, len);
          string str = Encoding.UTF8.GetString(result_bytes);
          // string str = Encoding.GetEncoding("gb2312").GetString(result_bytes);
          
          if (count > 0 && imgs != IntPtr.Zero)
          {
            VzClientSDK.VZ_IMAGE_INFO[] struct_imgs = new VzClientSDK.VZ_IMAGE_INFO[count];
            for (int i = 0; i < count; i++)
            {
              IntPtr ptr = new IntPtr(imgs.ToInt64() + i * Marshal.SizeOf(typeof(VzClientSDK.VZ_IMAGE_INFO)));
              struct_imgs[i] = (VzClientSDK.VZ_IMAGE_INFO)Marshal.PtrToStructure(ptr, typeof(VzClientSDK.VZ_IMAGE_INFO));

              // 示例, 只取大图片
              if (struct_imgs[i].img_len > 0 && i == 0)
              {
                int img_len = (int)struct_imgs[i].img_len;
                byte[] picdata = new byte[img_len];
                Marshal.Copy(struct_imgs[i].buffer, picdata, 0, img_len);
        ...
        private void FIND_DEVICE_CALLBACK_EX(string pStrDevName, string pStrIPAddr, ushort usPort1, ushort usPort2, uint SL, uint SH, string netmask, string gateway, IntPtr pUserData)
        {
            ...
            Win32API.PostMessage(hwndMain, MSG_DEVICE_INFO, intptr, (IntPtr)0);
        }
        ...
        private void recordbtn_Click(object sender, EventArgs e)
        {
            ...
            resetrecordtime = new System.Timers.Timer(1000 * 60 * 2);
            ...
            string szPath = strFilePath + "\\" + DateTime.Now.Year.ToString() +
                ...
                ".avi";
            int ret = VzClientSDK.VzLPRClient_SaveRealData(lprHandle, szPath);
            recordbtn.Enabled = false;
            stoprecordbtn.Enabled = true;
            record_handle_ = lprHandle;
        }
        ...
        private void snapshootbtn_Click(object sender, EventArgs e)
        {
            ...
            if( m_bFirst )
            {
                ...
                VzClientSDK.VzClient_SetCommonResultCallBack(lprHandle, 1, m_common_recv, IntPtr.Zero);
            }
            else 
            {
                pictureBox3.Image = null;
        ...
        protected override void DefWndProc(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                case MSG_DEVICE_INFO:
                    intptr = (IntPtr)m.WParam.ToInt64();
                    if (intptr != null)
                    {
