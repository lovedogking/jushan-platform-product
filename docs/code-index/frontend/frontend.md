...
> **最近更新**：2026-07-26（v2.6：岗亭开闸改为抓拍→记日志→直接开闸，人工放行独立按钮并删道闸常关；新增操作日志页；人工放行弹窗加车牌颜色/车辆类型选择+自动抓拍+语音播报；通行记录可点击在左下角显示详情含双图+出入场时间；余位调整简化为仅直接设定；状态标签「在场」→「已入场」；记录表加水平滚动条）
