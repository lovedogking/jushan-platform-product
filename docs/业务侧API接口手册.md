...
所有道闸控制异步执行，响应 `data.success` 表示命令是否下发成功。

### 10. 开闸

向摄像头发送 IO0 继电器脉冲信号（先通后断，1500ms），道闸抬起。

```
POST /api/v1/devices/{deviceId}/gate/open
```

请求体为空 `{}`。要求设备产品目录声明 `OPEN_GATE` 能力。

### 11. 关闸

向摄像头发送 IO1 继电器脉冲信号（先通后断，3000ms），道闸落下。

```
POST /api/v1/devices/{deviceId}/gate/close
```

请求体为空 `{}`。要求设备产品目录声明 `CLOSE_GATE` 能力。

### 12. 锁定开闸

锁定 IO0 继电器强制吸合，保持道闸持续开启。用于临时管控场景（如消防通道常开）。

```
POST /api/v1/devices/{deviceId}/gate/lock
```

请求体为空 `{}`。要求设备产品目录声明 `LOCK_OPEN_GATE` 能力。

> **注意**：臻识 C5H 硬件仅支持锁定开闸方向。锁定关闸因 IO0/IO1 无电气互锁不可行，该能力已在 v0.4 移除。

### 13. 解除锁定并关闸

解锁 IO0 继电器 + 重置道闸状态机 + 执行关闸脉冲，使道闸恢复到关闭状态。

```
POST /api/v1/devices/{deviceId}/gate/unlock
```
...
道闸控制
  ...
  POST   /api/v1/devices/{deviceId}/gate/lock           锁定开闸（继电器强制吸合保持开启）
  POST   /api/v1/devices/{deviceId}/gate/unlock         解除锁定并关闸
