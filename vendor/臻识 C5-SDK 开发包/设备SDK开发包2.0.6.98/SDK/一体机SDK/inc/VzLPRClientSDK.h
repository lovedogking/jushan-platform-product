...
*  @param  [IN] wRtspPort 流媒体的端口号,为0表示使用默认
*  @param  [IN] wTcpPort tcp端口号,为0表示使用默认
*  @param  [IN] wHttpType http方式(0:http, 1:https)
*  @return 返回设备的操作句柄，当打开失败时，返回0
*  @ingroup group_device
*/
VZ_LPRC_API VzLPRClientHandle __STDCALL VzLPRClient_OpenEx3(const char *pStrIP, WORD wPort, const char *pStrUserName, const char *pStrPassword,WORD wRtspPort, WORD wTcpPort, WORD wHttpType);
...
*  @param  [IN] pStrUserName 访问设备所需用户名
*  @param  [IN] pStrPassword 访问设备所需密码
*  @param  [IN] pRtspIP RTSP的IP
*  @param  [IN] wRtspPort 流媒体的端口号,为0表示使用默认
*  @param  [IN] pTcpIP Tcp的IP
*  @param  [IN] wTcpPort tcp端口号,为0表示使用默认
*  @param  [IN] wHttpType http方式(0:http, 1:https)
*  @return 返回设备的操作句柄，当打开失败时，返回0
*  @ingroup group_device
*/
VZ_LPRC_API VzLPRClientHandle __STDCALL VzLPRClient_OpenEx4(const char *pStrIP, WORD wPort, const char *pStrUserName, const char *pStrPassword,const char *pRtspIP,WORD wRtspPort, const char *pTcpIP, WORD wTcpPort, WORD wHttpType);
...
*  @brief 获取系统日志
*  @param  [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] iLogType 日志的类型(业务日志:2, 系统日志:4, 异常日志:8, 操作日志:16, 全部日志:30)
...
/**
*  @brief 设置用户自定义OSD配置(支持八行)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] pOsdParam 用户自定义osd配置
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetUserOsdParamEx2(VzLPRClientHandle handle, VZ_LPRC_USER_OSD_EX2_PARAM* pOsdParam);

/**
*  @brief 获取用户自定义OSD配置(支持八行)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [OUT] pOsdParam 用户自定义osd配置
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_GetUserOsdParamEx2(VzLPRClientHandle handle, VZ_LPRC_USER_OSD_EX2_PARAM* pOsdParam);

/**
*  @brief 设置动态OSD参数(不写配置文件)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] pOsdParam 用户自定义osd配置
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetUserDynamicOsdParam(VzLPRClientHandle handle, VZ_LPRC_USER_OSD_EX2_PARAM* pOsdParam);
...
/**
*  @brief 开始录像功能(录制mp4文件格式)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] sFileName   录像文件的路径
*  @param [IN] stream_type 码流类型(0主码流, 1子码流)
*  @return 返回值录像句柄，返回-1表示失败
*  @ingroup group_device
*/
VZ_LPRC_API int __STDCALL VzLPRClient_StartRecord(VzLPRClientHandle handle,char *sFileName, int stream_type);

/**
*  @brief 停止录像(录制mp4文件格式)
*  @param [IN] record_handle 由VzLPRClient_StartRecord函数获得的句柄
*  @return 返回值为0表示成功，返回-1表示失败
*  @ingroup group_device
*/
VZ_LPRC_API int __STDCALL VzLPRClient_StopRecord(int record_handle);
...
*  @param [IN] img_size图像大小(2:352*288 3: 640*360 5:704*576 9:1280*720 10:1920*1080 11:2304*1296 12:2560*1440 15:2880*1620)
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
...
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzClient_QueryRecordVideo(VzLPRClientHandle handle, int sub_type, const char* start_time, const char* end_time, int page_num, VZ_FIC_RECORD_VIDEO_INFO* record_video);
...
/**
*  @brief 识别结果回调
*  @param [OUT] type结果类型(0:车道监控结果,1:三地车牌识别结果, 2:x3相机识别结果,3:车牌识别结果,支持二值化图像输出, 4: S5L人脸抓拍结果)
*  @param [OUT] pResultInfo结果信息(JSON格式字符串)
*  @param [OUT] len结果信息长度
*  @param [OUT] imgs图片信息
*  @param [OUT] count图片个数
*  @param [OUT] pUserData 用户自定义数据
...
*  @brief 设置通用识别结果回调
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] type 结果类型(0:车道监控结果,1:车牌识别结果(json字符串), 2:x3相机识别结果, 3:车牌识别结果,支持二值化图像输出, 4: S5L人脸抓拍结果)
*  @param [IN] func 回调接口
*  @param [IN] pUserData 用户自定义数据
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
...
*  @param  [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] netcfg 参见VZ_LPRC_NETCFG结构体定义
*  @param [IN] source 网口号, 第一个网口填0, 第二个网口填1
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetNetConfigV2(VzLPRClientHandle handle, VZ_LPRC_NETCFG* netcfg, int source);
...
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_GetNetConfigV2(VzLPRClientHandle handle, VZ_LPRC_NETCFG* netcfg, int source);


VZ_LPRC_API int __STDCALL VzLPRClient_ChangeIPv6NetworkParam(const char *src_id, const char* ip, const char* gateway, const char* netmask, const char* ex_data);


/**
*  @brief 根据通道号和码流id播放实时视频
*  @param  [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param  [IN] hWnd 窗口的句柄
*  @param  [IN] channel 通道号(针对盒子设备可以接多路相机, 其他设备通道号填0)
*  @param  [IN] stream_id 码流id(0主码流, 1子码流, 2:x3相机车头设备主码流, 3:x3相机车头设备子码流, 4:x3相机车尾设备主码流, 5:x3相机车尾设备子码流,
                                 6:X2相机车道主码流,7:X2相机车道子码流)
*  @return 播放的句柄，-1表示失败
*  @ingroup group_device
*/
VZ_LPRC_API int __STDCALL VzLPRClient_StartRealPlayByChannel(VzLPRClientHandle handle, void *hWnd, int channel, int stream_id);


/**
*  @brief 设置图片OSD参数
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] pOsdParam 图片OSD参数
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetImgOsdParam(VzLPRClientHandle handle, VZ_LPRC_IMG_OSD_PARAM* pOsdParam);

/**
*  @brief 获取图片OSD参数
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [OUT] pOsdParam 图片OSD参数
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_GetImgOsdParam(VzLPRClientHandle handle, VZ_LPRC_IMG_OSD_PARAM* pOsdParam);

/**
*  @brief 设置图片OSD参数(支持八行自定义)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] pOsdParam 图片OSD参数
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetImgOsdParamEx(VzLPRClientHandle handle, VZ_LPRC_IMG_OSD_PARAM_EX* pOsdParam);

/**
*  @brief 获取图片OSD参数(支持八行自定义)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [OUT] pOsdParam 图片OSD参数
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_GetImgOsdParamEx(VzLPRClientHandle handle, VZ_LPRC_IMG_OSD_PARAM_EX* pOsdParam);


/**
*  @brief 设置是否输出日志
*  @param [IN] log_path   日志文件的路径(指定的路径必须存在,工具不会创建文件夹)
*  @param [IN] enable_log 1:输出日志 0:不生成日志
*  @ingroup group_global
*  @return 返回值为0表示成功，返回-2代表指定的路径不存在, -1为其他错误
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetOutputLog(const char* log_path, int enable_log);

/**
*  @brief 设置车牌解密密码(车牌加密的情况下,自动解密车牌号)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] szPwd 解密的密码
*  @ingroup group_global
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetDecPwd(VzLPRClientHandle handle,const char *szPwd);

/**
*  @brief 获取存储在相机中的授权码
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [OUT] authorize_code 授权码信息
*  @param [IN] code_max_Len 授权码字符串最大长度
*  @ingroup group_global
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_GetAuthorizeCode(VzLPRClientHandle handle, char *authorize_code, int code_max_Len);


/**
*  @brief 下载http文件
*  @param [IN] url http url
*  @param [IN] file_path 存储文件的路径
*  @param [IN] time_out  超时时间,范围(3000-600000)
*  @ingroup group_global
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_DownloadHttpFile(const char* url, const char* file_path, int time_out);
...
*  @param [IN]  ad_type(0自定义/1全屏/2半屏广告;-1代表不指定广告类型,获取当前正在使用的广告配置)
*  @param [OUT] config广告配置内容
*  @param [IN]  max_len传入广告内容的长度
*  @return 返回值大于0表示成功，代表屏显方案内容的长度;-2代表传入长度不够;返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_GetLcdContent(VzLPRClientHandle handle, int ad_type, char *config, int max_len);

/**
*  @brief 设置LCD屏显方案内容
*  @param [IN] config屏显方案内容
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetLcdContent(VzLPRClientHandle handle, const char *config);
...
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetAuthorizeCode(VzLPRClientHandle handle, const char *data);

/**
*  @brief 设置用户自定义OSD配置(支持八行,每个自定义位置可配置)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [IN] pOsdParam 用户自定义osd配置
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_SetUserOsdParamEx3(VzLPRClientHandle handle, VZ_LPRC_USER_OSD_EX3_PARAM* pOsdParam);

/**
*  @brief 获取用户自定义OSD配置(支持八行, 每个自定义位置可配置)
*  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
*  @param [OUT] pOsdParam 用户自定义osd配置
*  @return 返回值为0表示成功，返回其他值表示失败。
*/
VZ_LPRC_API int __STDCALL VzLPRClient_GetUserOsdParamEx3(VzLPRClientHandle handle, VZ_LPRC_USER_OSD_EX3_PARAM* pOsdParam);
