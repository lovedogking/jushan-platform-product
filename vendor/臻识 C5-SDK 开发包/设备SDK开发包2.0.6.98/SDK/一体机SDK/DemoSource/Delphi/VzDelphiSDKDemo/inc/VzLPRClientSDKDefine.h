...
{
	unsigned				uNumVirtualLoop;	/**<实际个数*/
	VZ_LPRC_VIRTUAL_LOOP_EX	struLoop[VZ_LPRC_VIRTUAL_LOOP_MAX_NUM];	/**<虚拟线圈数组*/
}
...
typedef struct _VZ_FS_INFO_EX {
	...
	unsigned char hw_version[LPRC_HW_VERSION_SIZE];	///< 硬件版本号,1001是老版，,新版第三位表示通道路数，最后一位表示hwflag（HwFlags的按位与）
...
#define VZ_LPRC_REC_AMPOL	    (1 << (LT_ARMPOL) | 1 << (LT_ARMPOL2))	/**<武警车*/
...
#define MAX_OutputConfig_Len 7

//输出配置信息
typedef struct
{
	VZ_LPRC_OutputConfig oConfigInfo[MAX_OutputConfig_Len];	//多个输出配置输出的消息
}VZ_OutputConfigInfo;
