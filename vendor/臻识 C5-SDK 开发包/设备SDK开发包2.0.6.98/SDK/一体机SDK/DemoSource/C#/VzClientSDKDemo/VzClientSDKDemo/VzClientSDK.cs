...
namespace VzClientSDKDemo
{
    class VzClientSDK
    {
        ...
        public const int MAX_OutputConfig_Len = 9;
        //输出配置信息
        ...
        [StructLayout(LayoutKind.Sequential)]
        public struct VZ_IMAGE_INFO
        {
            public uint img_format;
...
//        public static extern int VzLPRClient_GetSnapShootToJpeg2(int nPlayHandle, string pFullPathName, int nQuality);
        ...
        */
        public delegate void VZ_COMMON_RESULT_CALLBACK(int handle, int type, IntPtr pResultInfo, int len, IntPtr imgs, int count, IntPtr pUserData);

        /**
        *  @brief 设置通用识别结果回调
        *  @param [IN] handle 由VzLPRClient_Open函数获得的句柄
        *  @param [IN] type 结果类型(0:车道监控结果,1:三地车牌识别结果, 2:x3相机识别结果)
        *  @param [IN] func 回调接口
        *  @param [IN] pUserData 用户自定义数据
        *  @return 返回值为0表示成功，返回其他值表示失败。
        */
        [DllImport("VzLPRSDK.dll")]
        public static extern int VzClient_SetCommonResultCallBack(int handle, int type, VZ_COMMON_RESULT_CALLBACK func, IntPtr pUserData);
    }
}
