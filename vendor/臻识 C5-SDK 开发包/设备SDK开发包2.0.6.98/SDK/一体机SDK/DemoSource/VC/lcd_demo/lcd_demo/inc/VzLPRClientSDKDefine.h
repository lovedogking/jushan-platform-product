...
typedef enum {
  ...
  VZ_LPRC_RESULT_RETENTION_TRIGGER = 64,
  VZ_LPRC_RESULT_RELIEVE_TRIGGER = 65,
...
#define MAX_USER_OSD_RICH_ITEM_COUNT 8
typedef struct
{
	short x_pos;        // 区域相对位置，取值范围[0,100]
	short y_pos;        // 区域相对位置，取值范围[0,100]
	int display;        // 是否显示
	BYTE color;       // 字体颜色 0：白色 1：红 2：蓝 3：绿 
	BYTE front_size;  // 字体大小 0:16号  1:24号  2:36号  3:48号
	VZ_LPRC_USER_OSD_ITEM_PARAM osd_item[MAX_USER_OSD_RICH_ITEM_COUNT];
}VZ_LPRC_USER_OSD_RICH_PARAM;
...
typedef struct
{
	...
	int y_pos; // 区域相对位置，取值范围[0,100]
	VZ_LPRC_USER_OSD_ITEM_PARAM osd_item[MAX_USER_OSD_EX_ITEM_COUNT];
	VZ_LPRC_USER_OSD_RICH_PARAM rich_item;
}VZ_LPRC_USER_OSD_EX2_PARAM;
...
}VZ_LPRC_USER_OSD_ITEM_PARAM_EX;

typedef struct
{
	int x_pos; 
	int y_pos; 
	VZ_LPRC_USER_OSD_ITEM_PARAM_EX osd_item[MAX_USER_OSD_EX_ITEM_COUNT];
	VZ_LPRC_USER_OSD_RICH_PARAM rich_item;
}VZ_LPRC_USER_OSD_EX3_PARAM;


#define MAX_IMG_OSD_TEXT_LEN 128

typedef struct
{
	BYTE id;     // 0 实时结果 1日期 2时间 3地点 4车辆事件 5车辆类型 6车标车款 7车身颜色 8自定义内容一 9自定义内容二 10自定义内容一
	BYTE enable; // 是否显示
	int x_pos; 
	int y_pos; 
	int type;       // 类型
	int ms_enable;  // 启用毫秒
	char text[MAX_IMG_OSD_TEXT_LEN];  // 文字
	char reserve[128];

}VZ_LPRC_IMG_OSD_ITEM_PARAM;

#define MAX_IMG_OSD_ITEM_COUNT 16
typedef struct
{
	BYTE pos;
	BYTE separation;
	...
	BYTE font_size;
	int osd_count;
	int content_order[MAX_IMG_OSD_ITEM_COUNT];
	int is_dynamic;
	char reserve[124];
	VZ_LPRC_IMG_OSD_ITEM_PARAM osd_item[MAX_IMG_OSD_ITEM_COUNT];
}VZ_LPRC_IMG_OSD_PARAM;


// 扩展图片osd
typedef struct
{
	BYTE id;     // 0 实时结果 1日期 2时间 3地点 4车辆事件 5车辆类型 6车标车款 7车身颜色 8自定义内容
	BYTE enable; // 是否显示
	BYTE ms_enable; 
	int type;       // 类型
	char text[MAX_IMG_OSD_TEXT_LEN];  // 文字
	char reserve[128];
}VZ_LPRC_IMG_SUB_PARAM;
...
typedef struct
{
	...
	int sub_count;
	char text[MAX_IMG_OSD_TEXT_LEN];  // 文字
	VZ_LPRC_IMG_SUB_PARAM sub_items[MAX_IMG_OSD_ITEM_COUNT]; // 子项内容数组
	char reserve[128];

}VZ_LPRC_IMG_OSD_ITEM_PARAM_EX;
...
typedef struct
{
	...
	int is_dynamic;
	char reserve[128];
	VZ_LPRC_IMG_OSD_ITEM_PARAM_EX osd_item[MAX_IMG_OSD_ITEM_COUNT];
}VZ_LPRC_IMG_OSD_PARAM_EX;
...
/**<武警车*/
#define VZ_LPRC_REC_AMPOL     (64|32768|65536|131072)  
...
#define VZ_LPRC_REC_ENERGY    (524288|1048576)  /**<新能源*/
...
//算法规则相关
#define MAX_ALG_POINT_COUNT 32
#define MAX_ALG_RULE_COUNT 20
typedef struct _Alg_COLOR_ATTR
{
	char dot_color[64];
	char line_color[64];
	char font_color[64];
	char fill_color[64];
	int line_type;  // 0：实线 1：虚线
}Alg_COLOR_ATTR;

typedef struct _Alg_RULE
{
	Alg_COLOR_ATTR normal_color;
	Alg_COLOR_ATTR active_color;
	...
	char rule_name[128];
	int point_num;
	ALG_POINT points[MAX_ALG_POINT_COUNT];
}Alg_RULE;

typedef struct _Alg_RULES
{
	int rule_num;
	Alg_RULE rules[MAX_ALG_RULE_COUNT];
}Alg_RULES;
...
// 录像文件列表查询
#define MAX_RECORD_VIDEO_NUM  100
typedef struct {
	char    file_name[124];
	int     channel;
	char    begin_time[24];
...
typedef struct {
	int     total;
	int     res_size;
	VZ_FIC_RECORD_VIDEO record[MAX_RECORD_VIDEO_NUM];
...
typedef struct {
	int elem_id;                // 元素id,从0开始累加计数
	int elem_enable;            // 是否启用
	char elem_name[32];         // 元素名称
	char elem_type[16];         // 元素类型
	char elem_sub_type[32];     // 元素子类型
	int picture_max_duration;   // 图片场景最长显示时间，单位s秒,范围(3-30)
	int video_play_count;       // 视频播放次数(1-10)
	char res_name[96];          // 视频或图片名称
	VZ_SCENE_POS pos;           // 位置
	char dwRes[256];            // 保留
...
typedef struct {
	...
	char elem_type[16];         // 元素类型
	char elem_sub_type[32];     // 元素子类型
	char elem_text_content[96]; // 文字内容
	char elem_bg_color[32];     // 背景色
	char elem_fg_color[32];     // 文字颜色
	int elem_font_family;       // 字体
	int elem_font_bold;         // 是否加粗
	int elem_font_size;         // 字号
	char elem_align[16];        // 对齐方式
	char extern_text[96];       // 扩展文字
	int extern_text_enable;     // 是否启用扩展文字
	VZ_SCENE_POS pos;           // 位置
	char res_name[96];          // 视频或图片名称
	char dwRes[256];            // 保留
...
/**广告页信息*/
typedef struct {
	char scene_name[64];                           // 场景名称
	int scene_id;                                  // 场景id
	int scene_mode;                                // 预留:场景模式
	char scene_type[32];                           // 预留:ad/car_in/pay/car_out/custom/idle
	int scene_max_duration;                        // 场景最大播放时长，单位s秒
	int video_play_count;                          // 视频播放次数
	int background_type;                           // 0表示背景色，1表示背景图，2代表不启用
	char background_color[32];                     // 背景颜色
	char background_image[96];                     // 图片名称
	VZ_SCENE_POS pos;                              // 位置
	VZ_SCENE_RES_ELEM res_elem[MAX_ELEM_COUNT];    // 图片或视频元素数组
	int res_elem_count;                            // 元素个数
	VZ_SCENE_TEXT_ELEM text_elem[MAX_ELEM_COUNT];  // 文字元素数组 
	int text_elem_count;                           // 文字元素个数
	char dwRes[256];                               // 保留
}VZ_SCENE_INFO;
...
typedef struct
{
	VZ_SCENE_INFO scene_info[5];     // 场景数组 
	int scene_count;                 // 场景个数
	int ad_type;                     // 0:自定义广告, 1:全屏广告, 2: 半屏广告
	int ratio;                       // 上半屏比例
}
VZ_AD_CONFIG;
...
#define MAX_MQTT_TOPIC_COUNT 64

typedef struct
{
	TOPIC_LIST_ITEM items[MAX_MQTT_TOPIC_COUNT];
	int item_count;
	char res[256]; 
}MQTT_TOPIC_LIST;
...
typedef struct
{
	...
	int heart_interval;
	int mqtt_type;
	char server_addr[MAX_MQTT_LEN];
	int server_port;
	char server_pwd[MAX_MQTT_LEN];
	char server_username[64];
	int timeout;

	char oss_addr[MAX_MQTT_LEN];
	char oss_bucket[MAX_MQTT_LEN];
	char oss_download_addr[MAX_MQTT_LEN];
	int oss_id;
	char oss_key[MAX_MQTT_LEN];
	char oss_pwd[MAX_MQTT_LEN];
	char oss_secret[MAX_MQTT_LEN];
	char oss_username[MAX_MQTT_LEN];
...
#define  VZ_SET_RTSP_PORT              1014

// 获取车辆信息
#define  VZ_GET_CAR_INFO               1201

// 车牌编码
#define  VZ_GET_PLATE_ENCODE           1212
#define  VZ_SET_PLATE_ENCODE           1213

// mqtt topic参数
#define  VZ_GET_MQTT_TOPIC_LIST        1214
#define  VZ_SET_MQTT_TOPIC_LIST        1215

// mqtt配置
#define  VZ_GET_MQTT_CONFIG            1216
#define  VZ_SET_MQTT_CONFIG            1217
