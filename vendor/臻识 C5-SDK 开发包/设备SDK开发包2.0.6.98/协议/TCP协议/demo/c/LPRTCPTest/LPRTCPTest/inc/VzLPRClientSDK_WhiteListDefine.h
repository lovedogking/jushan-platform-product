...
typedef struct
{
	unsigned			uEnable;
	unsigned			uIsSeg;
	VZ_TM_WEEK_SEGMENT  struWeekSeg[VZ_LPR_WLIST_TIME_SEG_MAX_NUM];
	VZ_TM_RANGE			struRange;
}
VZ_TM_PERIOD_OR_RANGE;
...
typedef struct
{
	...
	VZ_TM		*pStruTMOverdule;								/**<该记录过期时间,为空表示没有过期时间*/
	unsigned	bUsingTimeSeg;									/**<是否使用时间段*/
	VZ_TM_PERIOD_OR_RANGE struTimeSegOrRange;					/**<时间段信息*/
