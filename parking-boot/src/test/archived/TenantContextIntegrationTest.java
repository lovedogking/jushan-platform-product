...
class TenantContextIntegrationTest extends TestcontainersBaseTest {
    ...
    void shouldRejectTenantUserWithMissingTenantIdInSession() throws Exception {
        // 获取租户管理员用户 ID
        SysUser tenantAdmin = sysUserMapper.selectByUsernameIgnoreTenant("13900000001");
    ...
    void shouldNotTrustTenantIdFromRequestParameters() throws Exception {
        ...
        SysUser created = sysUserMapper.selectByIdIgnoreTenant(empId);
    ...
    private void cleanupTestData() {
        ...
        jdbcTemplate.update(
                "DELETE epl FROM employee_parking_lot epl " +
                        ...
                        "WHERE pl.name LIKE ? OR pl.name LIKE ?",
                "数据范围%", "跨租户%");
        jdbcTemplate.update(
                "DELETE FROM parking_lot WHERE name LIKE ? OR name LIKE ?",
                "数据范围%", "跨租户%");
