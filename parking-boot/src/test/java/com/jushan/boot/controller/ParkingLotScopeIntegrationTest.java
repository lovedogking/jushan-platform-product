...
class ParkingLotScopeIntegrationTest extends TestcontainersBaseTest {
    ...
    void setUp() throws Exception {
        ...
        // 注册客户
        registerAndApprove("13988880001", "Scope测试租户有限公司", "Scope测试联系人");
        customerAdminToken = login("13988880001", "pass123");

        // 创建两个停车场
        parkingLotAId = createParkingLotViaApi(customerAdminToken, "Scope停车场A-授权", 100);
        parkingLotBId = createParkingLotViaApi(customerAdminToken, "Scope停车场B-非授权", 200);
    ...
    void crossTenantRejectedEvenWhenParkingLotIdExists() throws Exception {
        registerAndApprove("13988880002", "Scope租户2有限公司", "Scope租户2联系人");
        String cust2Token = login("13988880002", "pass123");
        Long lotInTenant2 = createParkingLotViaApi(cust2Token, "Scope租户2停车场", 100);
    ...
    private void deleteEmployeeParkingLotAuth(String phone) {
        SysUser user = sysUserMapper.selectOne(
                new LambdaQueryWrapper<SysUser>().eq(SysUser::getUsername, phone));
        if (user != null) {
    ...
    private void cleanupTestData() {
        ...
        for (String pattern : new String[]{"Scope"}) {
        ...
        vendorMapper.delete(new LambdaQueryWrapper<DeviceVendor>().like(DeviceVendor::getName, "Scope"));
        modelMapper.delete(new LambdaQueryWrapper<DeviceModel>().like(DeviceModel::getName, "Scope"));
    }
}
