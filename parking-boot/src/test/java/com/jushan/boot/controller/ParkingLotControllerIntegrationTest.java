...
class ParkingLotControllerIntegrationTest extends TestcontainersBaseTest {
    ...
    void shouldCreateParkingLot() throws Exception {
        ...
        String resp = mockMvc.perform(post("/admin/parking-lots")
                        ...
                        .content("{\"name\":\"测试停车场A\",\"address\":\"测试地址1号\"," +
                                "\"contactPhone\":\"010-12345678\",\"totalSpaces\":100," +
    ...
    void shouldCreateWithDefaults() throws Exception {
        ...
        mockMvc.perform(post("/admin/parking-lots")
                        .header("Authorization", "Bearer " + customerAdminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"默认配置停车场\",\"totalSpaces\":50}"))
                .andExpect(status().isOk())
    ...
    void shouldCreateWithZeroSpaces() throws Exception {
        ...
        mockMvc.perform(post("/admin/parking-lots")
                        .header("Authorization", "Bearer " + customerAdminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"零车位停车场\",\"totalSpaces\":0}"))
                .andExpect(status().isOk())
    ...
    void shouldListParkingLotsInTenant() throws Exception {
        ...
        // 创建另一个租户和停车场
        registerAndApprove("13900000002", "其他租户", "其他管理员");
        String otherToken = login("13900000002", "pass123");
        createParkingLotViaApi(otherToken, "其他租户停车场", 300);
    ...
    void shouldRejectCrossTenantDetail() throws Exception {
        ...
        // 创建另一个租户的停车场
        registerAndApprove("13900000003", "跨查租户", "跨查管理员");
        String otherToken = login("13900000003", "pass123");
        Long otherLotId = createParkingLotViaApi(otherToken, "他人停车场", 100);
    ...
    void shouldDisableByCustomerAdmin() throws Exception {
        ...
        mockMvc.perform(post("/admin/parking-lots/" + lotId + "/status")
                        ...
                        .content("{\"action\":\"DISABLED\",\"reason\":\"设备维护\"," +
                                "\"disableNewEntries\":0,\"disablePayment\":1,\"disableExit\":1," +
                                "\"disableAutoGate\":0,\"disableOnlyConfig\":1}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(0));

        // 验证数据库
        ParkingLot disabled = parkingLotMapper.selectById(lotId);
        assertThat(disabled.getStatus()).isEqualTo("DISABLED");
        assertThat(disabled.getDisableNewEntries()).isEqualTo(0);

        // 验证审计日志
        Long logCount = statusLogMapper.selectCount(
                new LambdaQueryWrapper<ParkingLotStatusLog>()
                        .eq(ParkingLotStatusLog::getParkingLotId, lotId));
    ...
    void shouldReEnableParkingLot() throws Exception {
        ...
        ParkingLot enabled = parkingLotMapper.selectById(lotId);
    ...
    void shouldUpdateTotalSpaces() throws Exception {
        ...
        // 验证审计日志
        ParkingLotCapacityLog logEntry = capacityLogMapper.selectOne(
                new LambdaQueryWrapper<ParkingLotCapacityLog>()
                        .eq(ParkingLotCapacityLog::getParkingLotId, lotId)
                        .eq(ParkingLotCapacityLog::getFieldName, "total_spaces"));
    ...
    void shouldManuallyCorrectRemainingSpaces() throws Exception {
        ...
        ParkingLotCapacityLog logEntry = capacityLogMapper.selectOne(
                new LambdaQueryWrapper<ParkingLotCapacityLog>()
                        .eq(ParkingLotCapacityLog::getParkingLotId, lotId)
                        .eq(ParkingLotCapacityLog::getFieldName, "remaining_spaces"));
        assertThat(logEntry).isNotNull();
        assertThat(logEntry.getBeforeValue()).isEqualTo(100);
    ...
    private void cleanupTestData() {
        ...
        // 清理测试停车场
        for (String pattern : new String[]{"测试", "T18", "默认", "零车位", "负数", "列表",
                "详情", "他人", "原始", "部分", "可停", "可启", "双禁", "无原因", "管理员不可",
                "待启", "扩容", "修正", "负数容量", "无效字段", "缺原因", "权限测试", "平台可查",
                "跨查", "启用", "待停用"}) {
            java.util.List<ParkingLot> lots = parkingLotMapper.selectList(
                    new LambdaQueryWrapper<ParkingLot>().like(ParkingLot::getName, pattern));
            for (ParkingLot lot : lots) {
                // FIX-08：清理关联的车道和设备
                deviceMapper.delete(new LambdaQueryWrapper<Device>()
                        .eq(Device::getParkingLotId, lot.getId()));
                laneMapper.delete(new LambdaQueryWrapper<ParkingLane>()
                        .eq(ParkingLane::getParkingLotId, lot.getId()));
                capacityLogMapper.delete(new LambdaQueryWrapper<ParkingLotCapacityLog>()
                        .eq(ParkingLotCapacityLog::getParkingLotId, lot.getId()));
                statusLogMapper.delete(new LambdaQueryWrapper<ParkingLotStatusLog>()
                        .eq(ParkingLotStatusLog::getParkingLotId, lot.getId()));
