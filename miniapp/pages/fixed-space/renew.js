...
Page({
  ...
  async loadDetail() {
    ...
    try {
      var detail = await get('/api/v1/mini/fixed-spaces/' + this.data.passId)
      this.setData({
        plateNumber: detail.plateNumber || '',
        spaceNo: detail.spaceNo || '',
        parkingLotName: detail.parkingLotName || '未知车场',
        validEndDate: detail.validEnd || '',
      })
      this.calcAmount()
    } catch (err) {
  ...
  async doRenew() {
    ...
    try {
      await post('/api/v1/mini/fixed-spaces/' + this.data.passId + '/renew', {
